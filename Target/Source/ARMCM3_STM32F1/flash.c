...
/************************************************************************************//**
** \brief     Read, Modify and Write back data from flash memory. Note that the len
**            must be smaller than FLASH_ERASE_BLOCK_SIZE. Also it is important to use
**            FlashInit() before and FlashDone() after call. Otherwise
** \return    BLT_TRUE if successful, BLT_FALSE otherwise.
**
****************************************************************************************/
blt_bool FlashReadModifyWrite(blt_addr addr, blt_int32u len, blt_int8u *data)
{
  static blt_int8u data_block[FLASH_ERASE_BLOCK_SIZE*2];
  blt_int8u first_sector;
  blt_int8u last_sector;
  blt_addr start_addr;
  blt_addr end_addr;
  blt_int16u data_block_idx;
  blt_int16u i;
  blt_bool ret;
  blt_int8u *p_data;
  ...
  /* read before erase */
  CpuMemCopy((blt_addr)data_block, start_addr, end_addr - start_addr);
  // !!! Calculate base address for the desired page

  FlashInit();

  /* erase the data */
  ret = FlashErase(start_addr, (end_addr - start_addr));

  p_data = data;
  if(ret == BLT_TRUE)
  {
    data_block_idx = addr - start_addr;
    /* modify */
    for (i = 0; i < len; i++)
    {
      data_block[i+data_block_idx] = *p_data;
      p_data++;
    }
  }

  /* write back */
  ret = FlashWrite(start_addr, (end_addr - start_addr) + 1, data_block);
